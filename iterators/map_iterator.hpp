...
namespace ft {
	...
	protected:
		void setFirst(Node *root) {
			if (root) {
				for (; root->left && root->left->value != value_type(); root = root->left);
			}
			_node = root;
		}
	...
	protected:
		...
		Node *_lastNode;
	public:
		map_reverse_iterator(Node *node = 0, const key_compare &comp = key_compare())
				: _comp(comp) {
			setFirst(node);
			setLast(node);
		}
		map_reverse_iterator(const map_iterator<Key, T, Compare, Node>& copy) {
			_node = copy.getNode();
			_lastNode = copy.getLastNode();
			_comp = copy.getCompare();
		}
		...
		bool operator==(const map_reverse_iterator& it) const   { return (it._node == _node); }
		bool operator!=(const map_reverse_iterator& it) const   { return (it._node != _node); }
	...
	protected:
		...
		void setLast(Node *root) {
			if (root) {
				for (; root->right && root->right->value != value_type(); root = root->right);
			}
			_node = root;
		}
		Node* searchMaxNode(Node *root)
